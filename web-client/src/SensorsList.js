        ...
        !!data.heartRateData && data.heartRateData.length > 0 ? (
          <div>
            <div>
              ...
              {data.heartRateData[data.heartRateData.length - 1].value} BPM
            </div>
            <div>
              Average heart rate:{" "}
              {averageData(data.heartRateData.map(x => x.value))} BPM
            </div>
          </div>
        ) : (
          "No data"
...
export default function SensorsList({ data }) {
  ...
  return <div className={classes.root}>{getCards(data, classes)}</div>;
}
